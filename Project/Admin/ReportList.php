    ...
    <body>
        ...
        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
			...
			<?php
				...
				$i=0;
			 $selQry = "select * from tbl_product p inner join tbl_subcategory sc on sc.subcategory_id=p.subcategory_id inner join tbl_category c on c.category_id=sc.category_id inner join tbl_brand b on b.brand_id=p.brand_id inner join tbl_shop s on s.shop_id=p.shop_id order by product_name";
				 ...
				 $rs = $Conn->query($selQry);
				   ...
				   {
					   $selQry3 = "select COALESCE(SUM(stock_quantity),0) as stock from tbl_stock where product_id='".$data["product_id"]."'";
					   
					   
					   $rs3 = $Conn->query($selQry3);
						$data3 = $rs3->fetch_assoc();
					   
					   $selQry2 = "select COALESCE(SUM(cart_quantity),0) as qty from tbl_booking b inner join tbl_cart c on c.booking_id=b.booking_id where product_id='".$data["product_id"]."' and  cart_status>0 ";
